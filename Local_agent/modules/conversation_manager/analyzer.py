...
LIGHT_SYSTEM = """你是会话状态分析器。根据「上一轮 Conversation State + 本轮用户与助手发言」，输出更新后的 JSON：
{
  ...
  "open_tasks": [{"id": "", "title": "", "detail": "", "status": "open"}],
  "important_files": ["path"]
}
规则：保留仍有效的旧状态；仅在有依据时更新；memory_candidates 只放真正长期有用的偏好/决策；无则空数组。只返回 JSON。"""

FULL_SYSTEM = """你是会话压缩与分析器。在上下文将尽时，一次输出：
{
  "conversation_summary": "供新上下文初始化的摘要",
  "conversation_state": {
    "current_project": "",
    "current_topic": "",
    "task_progress": "",
...
Open Tasks 只记录未完成工作，不要暗示自动执行。只返回 JSON。"""
...
class ConversationAnalyzer:
    ...
    async def run(
        ...
        *,
        ...
        return self._parse(mode, data, prev_state, prev_summary, prev_open_tasks or [])
    ...
    def _parse(
        ...
        for item in data.get("open_tasks") or []:
            if isinstance(item, dict) and (item.get("title") or item.get("detail") or item.get("id")):
                open_tasks.append(OpenTask.model_validate(item))
        if not open_tasks:
